...
export default function HomePage() {
  ...
  const handleBuscarCedula = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${baseUrl}/api/cedula/${cedula.trim()}`);
      const data = await res.json();
      if (data.status === 'exito') {
        setResCedula(data.resultados?.data || data.resultados);
      } else {
        setErrorCedula(data.mensaje || 'No se encontraron registros.');
      }
    } catch {
      setErrorCedula('Error al conectar con el servidor.');
    } finally {
  ...
  const handleBuscarPlaca = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.status === 'exito') {
        setResPlaca(data.resultados?.data || data.resultados);
      } else {
        setErrorPlaca(data.mensaje || 'No se encontraron datos para esta placa.');
      }
    } catch {
      setErrorPlaca('Error al conectar con el servidor.');
    } finally {
