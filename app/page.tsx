  ...
  const handleBuscarCedula = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (/^\d+$/.test(searchValue)) {
        // Usamos /api/cedula/ que es la ruta correcta de tu backend en Railway
        endpoint = `${baseUrl}/api/cedula/${encodeURIComponent(searchValue)}`;
      } else {
        endpoint = `${baseUrl}/api/nombre?q=${encodeURIComponent(searchValue.toUpperCase())}`;
      }
      ...
      <div style={{ backgroundColor: '#fff', padding: '30px', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', width: '100%', maxWidth: '540px' }}>
        <div style={{ display: 'flex', borderBottom: '2px solid #e5e7eb', marginBottom: '25px' }}>
          <button
            ...
            style={{
              flex: 1, padding: '12px', border: 'none', background: 'none', fontWeight: '600', fontSize: '1rem', cursor: 'pointer',
              color: tab === 'cedula' ? '#2563eb' : '#6b7280',
              borderBottom: tab === 'cedula' ? '3px solid #2563eb' : 'none', marginBottom: '-2px'
            }}
          >
          ...
          <button
            ...
            style={{
              ...
              borderBottom: tab === 'placa' ? '3px solid #2563eb' : 'none', marginBottom: '-2px'
            }}
          >
            🚗 Buscar Placa
          </button>
